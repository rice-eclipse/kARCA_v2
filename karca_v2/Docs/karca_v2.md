...
## Project Overview
kARCA 2.0 is an engine control system  that is part of Rice Eclipse's Engine Testing Setup. The controller interfaces directly with the engine through the [controller software](https://github.com/rice-eclipse/mc-firmware), which runs on an STM32F207 nucleo board. The user interfaces with the controller using a frontend JavaScript dashboard `Quonkboard` from which they can visualize the data collected and send commands to actuate valves and start the ignition sequence.

**For future versions, it is recommended to upgrade to an STM32F429 for more RAM and a hardware FPU for efficient FIR filtering. The user can also use the STM32F407VET6 if a fully integrated solution is preferred (ie: MCU embedded on the main PCB)**
![Engine Test Setup](readme_images/engine_test_setup.png)

## System Requirements
1. Collect data from 4 load cells, pressure transducers, and thermocouples at a 1kHz samplingfrequency
...
The 5V line is used for the remaining ciruitry. The power conversion is performed by the [AP62300](https://www.diodes.com/assets/Datasheets/AP62300_AP62301_AP62300T.pdf) synchronous buck converter.
...
![Power Regulation](Schematics/Regulators.png)
![Battery Monitoring](Schematics/Regulators_battery-monitoring.png)
...
![Load Cell Amplification](Schematics/Load_Cells_amplification.png)
...
The signal is amplified with a gain of 8 using an active filter with a cutoff frequency of $193Hz$. Therefore the full voltage range is from 0.96V to 4.8V, corresponding to a resolution of 0.31psi.

![Pressure Transducer Amplification](Schematics/Pressure-Transducers_amplification.png)
...
Thermocouples output a very small voltage signal that is proportional to the temperature difference between the measurement junction and a reference junction.
 ...
 ![Thermocouple Amplification](Schematics/Thermocouples_amplification.png)

### Analog-to-Digital Conversion and Processing
The analog signals from the thermocouple, load cell, and pressure transducer conditioning circuits are input to two [MCP3208](https://www.microchip.com/en-us/product/mcp3208) 12-bit ADCs. The driver current and battery monitoring circuit outputs are input to a third MCP3208 ADC. The outputs from the first two ADCs are sent to the microcontroller over an SPI bus. The output from the third ADC is sent to the microcontroller over another SPI bus. This allows for a faster sample rate as there are fewer channel scans needed for each sampling cycle.

These ADCs have a Signal to Noise and Distortion (SINAD) of 72dB, providing an ENOB of ~11.

**For future revisions, it is recommended to use the internal ADCs as these allow for easy channel scanning and DMA transactions**
 ### Valve Actuation
 Each valve is driven by the AO3400 mosfet. These mosfets have a very low $$R_{DS,on}$$, allowing sufficient current to flow when then gate is driven by 3.3V from the 
 STM32F207 GPIO. There are also green status LEDs that light up, when a valve is actuated.

 ![Driver Schematics](Schematics/Drivers_mosfet.png)
 ...
 ![Driver Current Sense](Schematics/Drivers_current-sense.png)
